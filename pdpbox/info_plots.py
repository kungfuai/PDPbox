...
def actual_plot_with_probs(
    ...
    probs: list or numpy.Array or pandas.Series
    ...
    .. code-block:: python
        ...
        fig, axes, summary_df = info_plots.actual_plot(
            probs=sklearn_model.predict_proba(X)[:, -1], X=titanic_data[titanic_features],
    ...
    .. code-block:: python
        ...
        fig, axes, summary_df = info_plots.actual_plot(
            probs=sklearn_model.predict_proba(X), X=otto_data[otto_features],
            feature='feat_67', feature_name='feat_67', which_classes=[1, 2, 3])

    """
    # check inputs
    probs = np.array(probs)
    if len(probs.shape) == 1 or probs.shape[1] in [1, 2]:
        n_classes = 2
    else:
